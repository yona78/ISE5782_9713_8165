...
class TubeTests {
	...
	public void testFindIntsersections() {
		...
		assertEquals(tube.findIntsersections(new Ray(new Point(-2, -0.1, 0), new Vector(1, 0.1, 1))), intsersections.add(new Point(1, 0.2, 3)), "Test when the ray crosses it failed");
		intsersections.remove(1);
		
		// TC02: Test when the ray is out of it
		if (tube.findIntsersections(new Ray(new Point(-2, -5, 0), new Vector(1, 0.1, 1))) != null)
			fail("Test when the ray is out of it failed");
		
	    // =============== Boundary Values Tests ==================
		
		// TC11: Test when the vertical ray crosses it throw the center
		assertEquals(tube.findIntsersections(new Ray(new Point(-2, 0, 1), new Vector(1, 0, 0))), intsersections.add(new Point(1, 0, 1)), "Test when the vertical ray crosses it throw the center failed");
		intsersections.remove(1);
		
		// TC12: Test when the ray crosses it throw the center
		assertEquals(tube.findIntsersections(new Ray(new Point(-2, 0, 0), new Vector(1, 0, 1))), intsersections.add(new Point(1, 0, 3)), "Test when the ray crosses it throw the center failed");
		intsersections.remove(1);
		
		// TC13: Test when the vertical ray crosses it XXX
		assertEquals(tube.findIntsersections(new Ray(new Point(-2, 0.1, 0), new Vector(1, 0, 0))), intsersections.add(new Point(1, 0.2, 3)), "Test when the ray crosses it failed");
		intsersections.remove(1);
		
		// TC14: Test when the ray tangent it
		assertEquals(tube.findIntsersections(new Ray(new Point(-1, -2, -1), new Vector(0, 1, 1))), intsersections, "Test when the ray tangent it failed");
		
		// TC15: Test when the vertical ray tangent it
		assertEquals(tube.findIntsersections(new Ray(new Point(-1, -2, 0), new Vector(0, 1, 0))), intsersections, "Test when the vertical ray tangent it failed");
		
		// TC16: Test when the ray climate to the center line in the tube
		if(tube.findIntsersections(new Ray(new Point(0.5, 0, 0), dir)) != null)
		...
		if(tube.findIntsersections(new Ray(new Point(2, 0, 0), dir)) != null)
		...
		if(tube.findIntsersections(new Ray(new Point(0, 0, 0), dir)) != null)
			fail("Test when the ray is the center line of the tube failed");
		
		// TC19: Test when the ray climate to the center line and on the side of the tube
		if(tube.findIntsersections(new Ray(new Point(1, 0, 0), dir)) != null)
